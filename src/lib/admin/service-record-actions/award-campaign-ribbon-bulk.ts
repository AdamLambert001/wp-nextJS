...
export type AwardCampaignRibbonBulkInput = {
  ribbonSlug: string;
  awardedAt: string;
  userIds: string[];
};

export async function awardCampaignRibbonBulk(
  ...
  if (userIds.length > 20) {
    throw new Error("You can only award campaign ribbons to up to 20 users at once");
  }
  ...
  for (const id of userIds) {
    ...
    if (campaignRib.some(([slug]) => slug === ribbonSlug)) {
      results.push({ id, ok: false, reason: "already_awarded" });
      continue;
    }
  ...
  const awarded = results.filter((row) => row.ok).length;
  return { results, awarded, skipped: results.length - awarded };
}
